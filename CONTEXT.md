...
**Bridge piece**:
The author-supplied geometry that joins one connector's stock theme cap/stand/socket (still drawn by the game's `StaticBuildingMeshBuilder` end-cap path — untouched) to the **Body piece**. Keyed by **(medium, role)** — e.g. `(Wire, Input)`, `(Fluid, Enabled)` — declared once and reused across every slot of that medium, with an optional **per-slot override**. A `Disabled` face gets **no** bridge (flat body face). A `Join` face gets a **seam piece** — the bridge for the `Join` role, medium-agnostic; each of the two joined pieces bakes its own half at the shared face. The framework selects each variant's bridges from its slot roles and bakes them into the **Composed model**.
