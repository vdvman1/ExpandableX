...
- **Singleton** — the standalone, 1-tile case. Carries **no join connectors**, so it is *not* a member of the network and behaves as an ordinary building until expanded. Comes as the **Default singleton** (below) and, optionally, a **Configurable singleton** family of id-encoded variants for 1-piece slot state. `StaticLayout`s also use a singleton (painter, each cutter size).
...
**Join connector**:
A dedicated connector type — distinct from the building's gameplay connectors (`BuildingItemInput/Output`, `BuildingSignalInput/Output`, etc.) — used exclusively to decide which adjacent pieces form one logical building. It is the connector type the network simulation (see **DynamicLayout**) is parameterised on, so the network groups pieces purely by where their join connectors line up. A piece carries a join connector on each face shared with an interior neighbour and **none** on outer faces (see **DynamicLayout** "Border closing").

Using a *dedicated* type — rather than reusing an existing connector type, which our `ConnectorSynthesizer`'s extension point would also allow — keeps the join semantics independent of the building's actual I/O: a mod's network fuses only its own pieces (different `DynamicLayout` families use different join types and their own network systems), and authors needn't sacrifice a gameplay connector for joining. Joins are about composition, not flow.
