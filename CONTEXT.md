...
**Composed model**:
The in-world model of a **synthesised** variant, built by the framework at session init from a **Body piece** plus the **Bridge piece**s of the connectors that variant actually carries, baked into a single `MainMeshLOD` (and the derived blueprint/preview/isolated meshes) via the game's own `MeshBuilder` / `BuildingMeshGenerator` path — the same combination the game uses to bake `MainMeshLOD`. **Opt-in**: a piece that declares no body/bridge pieces falls back to today's behaviour (the base definition's model, cloned wholesale). Composition applies to synthesised variants **only** — the `MatchesBase` combination, **Variant override** targets, and the **Default singleton** keep their own hand-authored models, so an author's pieces must visually match those at the boundary. Because bridges bake into the **local frame**, they rotate with `GridRotation` canonicalisation for free (a local-East bridge lands on world-North with the body). The base definition's **custom draw data** is preserved unchanged, so parts a simulation renderer animates from it (e.g. the painter's roller/hinge) carry over untouched — the body is the static housing only, and an optional custom **blueprint** model supplies the static forms of those parts that the auto-derived blueprint would omit. A partial step toward the full config-driven rendering of issue #8 (the 3D "9-slice" system stays future work). See [ADR-0016](docs/adr/0016-composed-variant-models-from-body-and-bridge-pieces.md).
_Avoid_: baked model, merged mesh, variant model

**Body piece**:
The author-supplied clean main model for a piece, with the per-connector attachment geometry **stripped out** — the geometry that used to bridge a connector's theme cap to the building body is removed so it can be added back (or omitted) per variant. Replaces the base definition's `MainMeshLOD` wholesale in a **Composed model**; required whenever any **Bridge piece** is declared. Distinct from the **Configurable base**, which defines connector *geometry/positions*, not visuals.
_Avoid_: main mesh, base model, hull

**Bridge piece**:
The author-supplied geometry that joins one connector's stock theme cap/stand/socket (still drawn by the game's `StaticBuildingMeshBuilder` end-cap path — untouched) to the **Body piece**. Keyed by **connector IO type + role** — e.g. `(BuildingSignalInput, Input)`, `(BuildingFluidJunction, Enabled)` — declared once and reused across every connector of that type, with an optional **per-slot override** (and a per-slot explicit local pivot for when the derived placement won't do). Keying on the `IBuildingIO` subtype rather than a fixed medium set means connector types added by other mods work, and an author may register against a base type (matches every subtype) or a specific subtype — the **most specific** registered type assignable from a connector wins. Each bridge is placed at its connector's pivot with the connector's `TileDirection` (canonical author frame: the connector on the game-**East**/+X face, the identity rotation), plus an optional model-specific offset (offsets can instead be authored into the mesh); a per-slot bridge may instead be authored **in place** in the building's model frame and used verbatim, for irregular static layouts where the canonical East face is awkward to reason about. A `Disabled` face gets **no** bridge (flat body face). A `Join` face gets a **seam piece** — the bridge for the `Join` role; each of the two joined pieces bakes its own half at the shared face. The framework selects each variant's bridges from its slot roles and bakes them into the **Composed model**.
_Avoid_: connector piece, port mesh, socket model, nub
...
**Focus piece**:
When the player single-clicks one member of a network, the whole network becomes the selection (one `Logical building`) **and** the clicked member is the *focus piece* — the within-selection target whose `Connector slot` config the per-building HUD shows. (Expansion is **not** focus-scoped: `Drag-handle expansion` acts on whichever face the player grabs across the whole selected building — see that term.) It is the canonical "drill into one piece of the one building" concept. A focus piece exists **only** from a single-click into a network, never from a mass/area gesture — those show the many-buildings HUD even when they happen to cover exactly one whole network. Transient, session-only state, never persisted, so it does not touch saves or blueprints and the no-per-instance-ids rule (blueprint stability) is untouched.
...
**Slot label**:
The player-facing name shown above a `Connector slot`'s role buttons in the per-building HUD. Labelled and ordered by layout kind: a `DynamicLayout` slot takes the **world-absolute compass direction** of the face its connector sits on (the piece's local face rotated by the placed building's `GridRotation`) — full compass words ("North", "East", …) — and slots are **ordered clockwise from North**; a `StaticLayout` slot takes an **author-supplied, building-relative** string (e.g. "Left Fluid") in **registration order**. Dynamic labels are geometry-derived because each 1×1 piece holds exactly one connector per face, so direction is a unique identity that also tracks rotation; static geometry is arbitrary (two connectors may share a world direction, and there is no face map), so the author names them and the label is fixed to the building's default orientation. Distinct from the slot's **`Id`**, an internal unpersisted key absent from the **Variant id** — so labels never touch blueprint identity. Raw English for now (translation deferred, [[project-ui-text-untranslated]]); the eventual on-model/overlay presentation is future work. See [ADR-0015](docs/adr/0015-slot-labels-compass-for-dynamic-author-for-static.md).
_Avoid_: slot name, connector caption, side label, direction tag

**Drag-handle expansion**:
The player-facing interaction model for changing a building's layout, and the **single control surface** for all expansion — it unifies static-`Sequence` and `Network` grow/shrink behind one gesture (issue #5). When a `Logical building` is selected, each growable outer face shows a directional **expansion handle**; dragging a handle **outward** grows and **inward** shrinks. The gesture is uniform across expansion kinds; only the *granularity* differs — a `Network` adds/removes one `Building` per dragged cell, whereas a `Sequence` **snaps to whichever step's footprint the cursor sits within** along the axis, flipping to the next step (in the drag's direction, skipping locked steps) only once the cursor passes the current step's bounds — so the cells-per-step ratio follows each step's actual extent, not a fixed 1:1. Either way the target `MetaBuildingDefinition`'s ghost previews the result. (Exact snap thresholds are tunable.) Expansion is **decoupled from the `Focus piece`**: handles act on the whole selected building (a face is grabbed directly in the world), so the focus piece governs only `Connector slot` config, not expansion. Anchoring to drawn handles also disambiguates "which building does this expansion attach to" — a problem `Build-and-detect` cannot solve when same-type buildings sit one cell apart.

**Per-piece-face vs whole-edge.** v1 is **per-piece-face** expansion: a drag changes one piece's face, with the drag *magnitude* running along that face's axis. **Whole-edge expansion** — one handle growing/shrinking the entire contiguous run of pieces perpendicular to the dragged face — is deferred to issue #15. Inward-drag shrink applies only where building material lies behind the face along the drag axis, and only to single-join ends (looped/branched shapes can't shrink piece-by-piece — issue #12); a handle lights only its live directions. The earlier per-face HUD **buttons** were scaffolding (the ADR-0002 stepping stone) and are **removed** once handles ship; `Connector slot` role buttons stay in the side panel.
...
- **Network** — unbounded multi-piece growth of a `DynamicLayout` into one connected network (the AND gate). Grow/shrink is **directed from a specific face** (surfaced via `Drag-handle expansion`), so the framework always knows which face of which building is being expanded; the gameplay connector on that face **translates along the grow axis** ("pinch and stretch") onto the new end, and reverses on shrink. Which faces a player may grow from is gated by a **shape-limit predicate** (`Line`, `Rectangle`, `Free`, … — framework-supplied or author-written, live-state aware, see **Condition**), and the whole result by a **building-wide predicate** (e.g. "≥1 `Output`") plus connectivity (a shrink may not disconnect the building).
